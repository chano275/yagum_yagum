...
possible_paths = [
    os.path.join(os.path.dirname(__file__), '..', 'fastapi', 'app', 'data', 'players', 'NC_players.csv'),
    os.path.join(os.path.dirname(__file__), '..', 'app', 'data', 'players', 'NC_players.csv'),
    os.path.join(os.path.dirname(__file__), '..', 'data', 'players', 'NC_players.csv'),
    'fastapi/app/data/players/NC_players.csv'
]
...
try:
    ...
    # 기존 선수 데이터 확인 (중복 방지)
    existing_players = session.query(models.Player).filter(models.Player.TEAM_ID == NC_TEAM_ID).all()
