...
async def read_team_players(
    team_id: int,
    db: Session = Depends(get_db)
):
    try:
        logger.info(f"팀 선수 목록 조회: 팀 ID {team_id}")
        ...
        logger.error(f"팀 선수 목록 조회 중 오류: {str(e)}")
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail=f"팀 선수 목록 조회 중 오류 발생: {str(e)}"
        )
...
# 팀 경기 일정 조회
@router.get("/{team_id}/games", response_model=List[team_schema.TeamGameScheduleInfo])
async def read_team_games(
    team_id: int,
    ...
    try:
        logger.info(f"팀 경기 일정 조회: 팀 ID {team_id}, 예정 경기만={upcoming_only}")
        ...
        if upcoming_only:
            today = datetime.now().date()
            games = [game for game in games if game.DATE >= today]
        ...
        logger.error(f"팀 경기 일정 조회 중 오류: {str(e)}")
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail=f"팀 경기 일정 조회 중 오류 발생: {str(e)}"
        )