...
# 이미지 파일 열기
image_format = input("이미지 파일 형식을 입력하세요(예시: jpg, png 등): ")
image_path = "image/" + input("image 폴더 내 이미지 파일명을 입력하세요(예시: test): ") + f".{image_format}"
files = {
...
# OCR 결과를 JSON 파일로 저장
result_name = image_path.split("/")[-1].split(".")[0]
